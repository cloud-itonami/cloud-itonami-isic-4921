(ns transitops.phase
  "Phase 0->3 staged rollout for the ISIC-4921 'Urban and suburban
  passenger land transport' (city bus, tram, light rail, taxi/
  rideshare dispatch) SCHEDULING/DISPATCH LOGISTICS COORDINATION actor.

    Phase 0  read-only              -- no writes, still governor-gated.
    Phase 1  assisted-logging       -- service-record logging allowed,
                                       every write needs human approval.
    Phase 2  assisted-coordination  -- adds dispatch scheduling and
                                       maintenance-order proposals,
    ...
    Phase 3  supervised auto        -- governor-clean, high-confidence
                                       ...
                                       auto-commits, at any phase, and
                                       a high-cost `:coordinate-
                                       maintenance-order` still
  ...
  it. `transitops.governor`'s own `always-escalate-ops` enforces the
  same invariant independently -- two layers, not one, agree on this."
  (:require [transitops.governor :as governor]))
...
(defn gate
  ...
  - `:flag-safety-concern` is never auto-eligible at any phase, so it
    ...
    governor doesn't). A high-cost `:coordinate-maintenance-order`
...
(defn verdict->disposition
  "Map a UrbanTransitDispatchGovernor verdict to a base disposition

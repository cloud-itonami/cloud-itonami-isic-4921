(ns transitops.operation
  "OperationActor -- one coordination request = one supervised actor
  run, expressed as a langgraph-clj StateGraph. The advisor
  (TransitDispatchAdvisor) is sealed into a single node (:advise); its
  proposal is ALWAYS routed through the UrbanTransitDispatchGovernor
  ...
  human dispatch/operations coordinator. The approver resumes with
  `{:approval {:status :approved}}` (or :rejected)."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [transitops.advisor :as advisor]
            [transitops.governor :as governor]
            [transitops.phase :as phase]
...
(defn build
  ...
  `transitops.store/Store`).
  opts:
    :advisor      -- a `transitops.advisor/Advisor` (default: mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; TransitDispatchAdvisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (advisor/-advise advisor store request)]
            {:proposal p :audit [(advisor/trace request p)]})))

      ;; UrbanTransitDispatchGovernor -- independent censor (separate system than the advisor).
      (g/add-node :govern
            ...
            (case disposition
              ...
              {:disposition :escalate
               :audit [{:t :approval-requested
                        :op (:op request) :route-id (:route-id request)
                        :reason (or reason
      ...
      ;; dispatch/operations coordinator resumes with :approval. Then
      ;; route commit/hold.
          ...
          (if (= :approved (:status approval))
            {:disposition :commit
             ...
             :audit [{:t :approval-granted :op (:op request)
                      :route-id (:route-id request) :by (:by approval)}]}
            {:disposition :hold
